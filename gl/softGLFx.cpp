...
CASoftGLESFixed::glFogx(GLenum pname, GLfixed param)
{
  switch(pname)
  {
    ...
    case GL_FOG_START:   state_.fog.start.value   = param; break;
    case GL_FOG_END:     state_.fog.end.value     = param; break;
    case GL_FOG_MODE:    state_.fog.mode.value    = param; break;
    default:
      setError(GL_INVALID_ENUM);
      return;
  };
...
CASoftGLESFixed::glFogxv(GLenum pname, const GLfixed * params)
{
  switch(pname)
  {
    ...
    case GL_FOG_START:   state_.fog.start.value   = params[0]; break;
    case GL_FOG_END:     state_.fog.end.value     = params[0]; break;
    case GL_FOG_MODE:    state_.fog.mode.value    = params[0]; break;
    case GL_FOG_COLOR:
...
#define INFINITE_LIGHT
  ...
  if(state_.fog.enabled == true)
  {
    ...
    //{
    //  case GL_LINEAR:
        f = (state_.fog.end - (0 - v.ve.z)) / (state_.fog.end - state_.fog.start);
    //    break;
    //  case GL_EXP:
    //    break;
    //  case GL_EXP2:

...
ee_to_gsBitBlt(uint32_t daddr, int dw, int dpsm, int dx, int dy, int w, int h, uint32_t source)
{
  ...
  switch(dpsm)
  {
    ...
    case GRAPH_PSM_4HL: // Textures only (palettized)
      gifData.addSendImage((void *)source, (w * h) >> 1);
      break;
  };
